...
router.post('/:id/chat/entrenar', authenticate_1.authenticate, async (req, res) => {
    try {
...
router.get('/:id/chats', authenticate_1.authenticate, async (req, res) => {
    try {
        const negocio = await negocioRepo.findById(req.params.id);
        if (!negocio) {
            res.status(404).json({ error: 'Negocio no encontrado' });
...
router.get('/admin/stats', authenticate_1.authenticate, async (req, res) => {
    try {
