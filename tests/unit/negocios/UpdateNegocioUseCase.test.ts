...
function makeMockRepo(negocio: NegocioDomain | null = baseNegocio): INegocioRepository {
  return {
    ...
    create: jest.fn(),
    update: jest.fn().mockResolvedValue({ ...baseNegocio, name: 'Nuevo Nombre' }),
    list: jest.fn(),
  }
}
...
describe('UpdateNegocioUseCase', () => {
  it('updates negocio when owner matches', async () => {
    ...
    const result = await useCase.execute('neg-1', { name: 'Nuevo Nombre' }, 'owner-1')
