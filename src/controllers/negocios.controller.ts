  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      if (err.message === 'FORBIDDEN') {
        res.status(403).json({ error: 'No tienes permiso para modificar este negocio' })
        return
      }
      res.status(400).json({ error: err.message })
      return
...
router.post('/:id/chat/entrenar', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/:id/chats', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    const negocio = await negocioRepo.findById(req.params.id)
    if (!negocio) {
      res.status(404).json({ error: 'Negocio no encontrado' })
...
router.get('/admin/stats', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
